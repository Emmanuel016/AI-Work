...
# Define the graph as in the previous questions
graph = {
...
def depth_first_search(graph, start, goal):
    ...
    while stack:
        ...
        if node not in visited:
            visited.add(node)
            neighbors = [neighbor for neighbor in graph[node]['neighbors']]
            stack.extend((neighbor, path + [node]) for neighbor in sorted(neighbors) if neighbor not in visited)
...
def breadth_first_search(graph, start, goal):
    ...
    while queue:
        ...
        if node not in visited:
            visited.add(node)
            neighbors = [neighbor for neighbor in graph[node]['neighbors']]
            queue.extend((neighbor, path + [node]) for neighbor in sorted(neighbors) if neighbor not in visited)
...
def uniform_cost_search(graph, start, goal):
    ...
    while priority_queue:
        ...
        if node not in visited:
            visited.add(node)
            neighbors = [(neighbor, cost + graph[node]['neighbors'][neighbor]) for neighbor in graph[node]['neighbors']]
            for neighbor, new_cost in sorted(neighbors):
                heapq.heappush(priority_queue, (new_cost, neighbor, path + [node]))
...
def greedy_search(graph, start, goal):
    ...
    while priority_queue:
        ...
        if node not in visited:
            visited.add(node)
            neighbors = [neighbor for neighbor in graph[node]['neighbors']]
            priority_queue.extend((graph[neighbor]['heuristic'], neighbor, path + [node]) for neighbor in sorted(neighbors) if neighbor not in visited)
...
goal_state = 'G'
dfs_result = depth_first_search(graph, 'S', goal_state)
bfs_result = breadth_first_search(graph, 'S', goal_state)
ucs_result = uniform_cost_search(graph, 'S', goal_state)
greedy_result = greedy_search(graph, 'S', goal_state)
a_star_result = a_star_search(graph, 'S', goal_state)
